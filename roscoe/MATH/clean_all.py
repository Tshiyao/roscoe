            ...
            with open(output_path, 'w', encoding='utf-8') as f:
                for idx, item in enumerate(dataset):
                    ...
                    for resp_idx, resp in enumerate(responses):
                        # 获取对应的result，如果存在
                        result = results[resp_idx] if resp_idx < len(results) else None
                        
                        # 构建新的JSON对象，key格式为dataset_key_idx_顺序数字
                        new_entry = {
                            "premise": question,
                            "hypothesis": f"IGNORE THIS. Ground truth here for reference. {answer}",
                            "gpt-3": resp,
                            "answer": answer,
                            "result": "yes" if result else "no",  # 新增字段保存对应的result值
                            "key": f"{dataset_key}_{idx}_{resp_idx}"
                        }
...
if __name__ == "__main__":
    ...
    output_directory = f"/root/autodl-tmp/roscoe/ParlAI/projects/roscoe/MATH/clean/{filename}/"
